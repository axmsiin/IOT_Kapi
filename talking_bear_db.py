...
# ============================================================
# MongoDB setup (อิงโครงจาก iot.py)
# ============================================================
...
weekly_summary_collection = None
monthly_summary_collection = None
...
def connect_mongodb():
    ...
    global users_collection, sessions_collection, interactions_collection
    global weekly_summary_collection, monthly_summary_collection

    try:
        mongo_client = MongoClient(MONGODB_URI)
        db = mongo_client[MONGO_DB]
        users_collection = db["bear_users"]
        sessions_collection = db["bear_sessions"]
        interactions_collection = db["bear_interactions"]
        weekly_summary_collection = db["bear_weekly_summary"]
        monthly_summary_collection = db["bear_monthly_summary"]
        ...
        sessions_collection.create_index([("person_id", 1), ("started_at", -1)])
        ...
        weekly_summary_collection.create_index([("person_id", 1), ("week_key", 1)], unique=True)
        monthly_summary_collection.create_index([("person_id", 1), ("month_key", 1)], unique=True)
        print(f"✅ Connected to MongoDB | DB={MONGO_DB}")
        print("🗂 collections: bear_users, bear_sessions, bear_interactions, bear_weekly_summary, bear_monthly_summary")
...
def choose_person():
    ...
    try:
        ...
        print("✅ บันทึกผู้ใช้และ session ลง MongoDB แล้ว")
...
def close_current_session():
    if current_session_id:
        sessions_collection.update_one(
            {"session_id": current_session_id, "ended_at": None},
            {"$set": {"ended_at": utcnow(), "status": "closed"}},
        )
...
def save_interaction_to_db(analysis_result, has_face, audio_file_size, image_file_size):
    ...
    update_weekly_summary(current_person_id, week_key)
    update_monthly_summary(current_person_id, month_key)


def update_weekly_summary(person_id, week_key):
    pipeline = [
        {"$match": {"person_id": person_id, "week_key": week_key}},
        {
            "$group": {
                "_id": {"person_id": "$person_id", "week_key": "$week_key"},
                "interaction_count": {"$sum": 1},
                "transcript_count": {
                    "$sum": {
                        "$cond": [{"$gt": [{"$strLenCP": {"$ifNull": ["$transcript", ""]}}, 0]}, 1, 0]
                    }
                },
                "avg_stress": {"$avg": "$symptom_scores.stress"},
                "avg_depression": {"$avg": "$symptom_scores.depression"},
                "avg_anxiety": {"$avg": "$symptom_scores.anxiety"},
                "avg_fatigue": {"$avg": "$symptom_scores.fatigue"},
                "last_created_at": {"$max": "$created_at"},
                "emotions": {"$push": "$face_analysis.label"},
            }
        },
    ]
    ...
    agg = result[0]
    weekly_summary_collection.update_one(
        {"person_id": person_id, "week_key": week_key},
        {
            "$set": {
                "person_id": person_id,
                "week_key": week_key,
                "interaction_count": agg.get("interaction_count", 0),
                "transcript_count": agg.get("transcript_count", 0),
                "avg_stress": agg.get("avg_stress"),
                "avg_depression": agg.get("avg_depression"),
                "avg_anxiety": agg.get("avg_anxiety"),
                "avg_fatigue": agg.get("avg_fatigue"),
                "emotion_labels": agg.get("emotions", []),
...
def update_monthly_summary(person_id, month_key):
    pipeline = [
        {"$match": {"person_id": person_id, "month_key": month_key}},
        {
            "$group": {
                "_id": {"person_id": "$person_id", "month_key": "$month_key"},
                "interaction_count": {"$sum": 1},
                "avg_stress": {"$avg": "$symptom_scores.stress"},
                "avg_depression": {"$avg": "$symptom_scores.depression"},
                "avg_anxiety": {"$avg": "$symptom_scores.anxiety"},
    ...
    agg = result[0]
    monthly_summary_collection.update_one(
        {"person_id": person_id, "month_key": month_key},
        {
            "$set": {
                "person_id": person_id,
                "month_key": month_key,
